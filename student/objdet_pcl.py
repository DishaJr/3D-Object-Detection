...
def show_pcl(pcl: np.ndarray):
    ...
    def right_click(vis_lpc):
        ...
        idx= False
        return
    vis_lpc.register_key_callback(262,close_window)
...
def show_range_image(frame: dataset_pb2.Frame, lidar_name: int) -> np.ndarray:
    ...
    deg90_1 = int(ri_range.shape[1] / 8)
    ri_center_1 = int(ri_range.shape[1]/2)
    ri_range = ri_range[:,ri_center_1-deg90_1:ri_center_1+deg90_1]
    
    img_range_intensity = np.vstack((ri_range,img_intensity))
