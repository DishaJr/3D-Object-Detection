...
def load_configs_model( model_name: str='darknet', configs: edict=None ) -> edict:
...
def create_model(configs: edict) -> torch.nn.Module:
...
def detect_objects(input_bev_maps: List[np.ndarray], model: torch.nn.Module, configs: edict) -> List[list]:
